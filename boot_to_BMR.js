...
function insertVirtualMediaCD(node_ip, img_path) {
	return new Promise((resolve, reject) => {
		console.log(`inserting VM-CD on ${node_ip}..`); //debugging
...
function ejectVirtualMediaCD(node_ip) {
	return new Promise((resolve, reject) => {
		console.log(`ejecting VM-CD from ${node_ip}..`); //debugging
...
function insertVmCdOnNodes(idrac_ips, image_path) {
	return new Promise((resolve, reject) => {
		console.log(`${COLOR.fgYellow}---insertVmCdOnNodes function called for ${idrac_ips} ---${COLOR.reset}`);   //debugging
		let insertedCounter = 0;
      ...
      let image_name = image_path.substring(image_path.lastIndexOf('/')+1);
			...
			idrac_ips.forEach(idrac_ip => {
				checkVirtualMediaCdStatus(idrac_ip)
                  ...
                  checkVirtualMediaCdStatus(idrac_ip)
                    .then(response => {
                      if (response.message === "true") {
                        console.log(`VM status check for ${idrac_ip}: did NOT eject!`);
                      } else {
                        console.log(`VM status check for ${idrac_ip}: ejected`);
                        insertVirtualMediaCD(idrac_ip, image_path)
                          .then(response => {
                            console.log(`insertVirtualMediaCD result for ${idrac_ip} is: ${response.success}`);
                            if (response.success) {
                              insertedCounter++;                              
                                if (insertedCounter == idrac_ips.length) {
                                  if (idrac_ips.length == 1)
			...
			console.log(`CATCH in insertVmCdOnNodes: ${error.message}`);
			reject({
				success: false,
				message: `CATCH in insertVmCdOnNodes: ${error.message}`
			});
		}
	});
}
...
// let img_path = "cifs://SVC_Auto_Image:MustChange2!@10.211.4.215/dropbox/dl/WIMs/XC/RASR_BMR_4.2_Test_Only_For_XC_Automation.iso";
...
//                           console.log(`PASS: Inserted Virtual Media on ${idrac_ip}`);
// 													insertedCounter++;
// 													if (insertedCounter === ip_arr.length) {
//                             console.log("Rebooting code..");
// 														// rebootSelectedNodes(ip_arr)
// 														// 	.then(response => {
// 														// 		if (response.success === true) {
...
// 								})
// 								.catch(error => {
// 									console.log(`FAIL: ejectVirtualMediaCD for ${idrac_ip} failed: ${error.message}`);
// 								})
...
// setInterval(() => {
//   insertVmCdOnNodes(ip_arr, img_path)
//   .then(response => {
//     console.log(`insertVmCdOnNodes result: ${response.message}`);
//     // console.log(`${COLOR.fgGreen}Rebooting code goes here..${COLOR.reset}`);
//     if (response.success) {
...
//   })
//   .catch(error => {
//     console.log(`FAIL: insertVmCdOnNodes result: ${error.message}`);
//   });
// }, 12000);


module.exports = { insertVmCdOnNodes, setNextOneTimeBootVirtualMediaDevice, rebootSelectedNodes };
