...
function insertVirtualMediaCD(node_ip, img_path) {
  return new Promise((resolve, reject) => {
    ...
    let payload = {};
    if (img_path.includes("cifs")) payload = { Image: img_path };
    else
      payload = {
        Image: img_path,
        Inserted: true,
...
function setNextOneTimeBootVirtualMediaDevice(idrac_ip) {
  return new Promise((resolve, reject) => {
    console.log(
      "setNextOneTimeBootVirtualMediaDevice function called for ",
      idrac_ip
    );
    ...
    let payload = {
      ...
      ImportBuffer:
        '<SystemConfiguration><Component FQDD="iDRAC.Embedded.1"><Attribute Name="ServerBoot.1#BootOnce">Enabled</Attribute><Attribute Name="ServerBoot.1#FirstBootDevice">VCD-DVD</Attribute></Component></SystemConfiguration>',
    };
...
function deleteJobQueue(node_ip, arg) {
  return new Promise((resolve, reject) => {
    console.log("deleteJobQueue function called for ", node_ip);
    ...
    let payload;
    if (arg.includes("JID_")) payload = { JobID: arg };
    else if (arg === "CLEARALL") payload = { JobID: "JID_CLEARALL" };
      ...
      .then((response) => {
        if (response.status != 200)
          ...
          if (arg.includes("JID_"))
            console.log(
              `- PASS: DeleteJobQueue action passed to clear job ID ${arg}, status code 200 returned`
            );
                ...
                .then((resObj) => {
                  if (resObj.status != 200) {
                    ...
                    if (lc_status === "Ready" && server_status === "Ready") {
                      ...
                      console.log(
                        "- WARNING, Lifecycle Controller services not in ready state, polling again"
                      );
                      sleep(20000);
                    }
                  }
  ...
  return new Promise((resolve, reject) => {
    ...
    idrac_ips.forEach((idrac_ip) => {
      checkRedfishSupport(idrac_ip)
        .then((response) => {
          console.log(
            `checkRedfishSupport result for ${idrac_ip} is: ${response.message}`
          );
          if (response.success) {
            checkAttachStatus(idrac_ip)
              .then((response) => {
                console.log(
                  `checkAttachStatus result for ${idrac_ip} is: ${response.message}`
                );
                if (response.message === "Attached") {
                  disconnectNetworkIsoImage(idrac_ip)
                    .then((response) => {
                      console.log(
                        `disconnectNetworkIsoImage result for ${idrac_ip} is: ${response.message}`
                      );
                      if (response.success) {
                        connectNetworkIsoImage(
                          ...
                          .then((response) => {
                            console.log(
                              `connectNetworkIsoImage result for ${idrac_ip} is: ${response.message}`
                            );
                            if (response.success) {
                              mountedCounter++;
                          ...
                          .catch((error) => {
                            console.log(
                              `CATCH in mountNetworkImageOnNodes: ${error.message}`
                            );
                            reject({
                              success: false,
    ...
    idrac_ips.forEach((idrac_ip) => {
      checkCurrentPowerState(idrac_ip)
        .then((response) => {
          if (response.success) {
            console.log(
              `checkCurrentPowerState result for ${idrac_ip} is: ${response.message}`
            );
            if (response.message == "On") rebootType = "ForceRestart";
            else rebootType = "PushPowerButton";
            console.log(`rebootType for ${idrac_ip} set to: ${rebootType}`);
            rebootSystem(idrac_ip, rebootType)
              .then((response) => {
                console.log(
                  `rebootSystem result for ${idrac_ip} is: ${response.message}`
                );
                if (response.success) {
                  rebootCounter++;
            ...
            console.log(
              `Could not get Power State from ${idrac_ip}, error: ${response.message}`
            );
            reject({
              success: false,
        ...
        .then((response) => {
          ...
          if (response.success) {
            checkVirtualMediaCdStatus(idrac_ip)
              .then((response) => {
                console.log(
                  `checkVirtualMediaCdStatus result for ${idrac_ip} is inserted: ${response.message}`
                );
                if (response.message === "true") {
                  ejectVirtualMediaCD(idrac_ip)
                    .then((response) => {
                      console.log(
                        `ejectVirtualMediaCD result for ${idrac_ip} is: ${response.message}`
                      );
                      if (response.success) {
                        insertVirtualMediaCD(idrac_ip, image_path)
                          .then((response) => {
                            console.log(
                              `insertVirtualMediaCD result for ${idrac_ip} is: ${response.message}`
                            );
                            if (response.success) {
                              deleteJobQueue(idrac_ip, "CLEARALL").then(
                                (response) => {
                                  console.log(
                                    `deleteJobQueue result for ${idrac_ip} is: ${response.message}`
                                  );
                                  if (response.success) {
                                    setNextOneTimeBootVirtualMediaDevice(
                                      idrac_ip
                                    ).then((response) => {
                                      console.log(
                                      ...
                                      if (!response.success) {
                                        ...
                                        insertedCounter++;
                                        if (
                                          insertedCounter == idrac_ips.length
                                        ) {
                                          if (idrac_ips.length == 1)
                                            resolve({
                                              success: true,
                          ...
                          .catch((error) => {
                            console.log(
                              `CATCH in insertVirtualMediaOnNodes: ${error.message}`
                            );
                            reject({
                              success: false,
                  ...
                  insertVirtualMediaCD(idrac_ip, image_path)
                    ...
                    .catch((error) => {
                      console.log(
                        `CATCH in insertVirtualMediaCD: ${error.message}`
                      );
                      reject({
                        success: false,
